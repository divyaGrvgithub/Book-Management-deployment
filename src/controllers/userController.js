...
const createUser = async (req, res) => { 
  try {
    ...
    //   return res.status(400).send({ status: false, message: "phone should be string" })
    // }
    ...
    if (!Validation.isValid(email)) {
      return res.status(400).send({ status: false, message: "email should be string" })
    }
    ...
    if (!Validation.isValid(password)) {
      return res.status(400).send({ status: false, message: "password should be string" })
    }
    ...
    if (!Validation.isValidPassword(password)) {
      return res.status(400).send({ status: false, message: "Password must be contains one special character, alphabet and number.It should be of 8-15 digits. " });
    }
    ...
    if (address) {
      ...
      if (street) {
        if (!Validation.isValid(street)) {
          return res.status(400).send({ status: false, message: "street should be string" })
        }
      ...
      if (city) {
        if (!Validation.isValid(city)) {
          return res.status(400).send({ status: false, message: "city should be string" })
        }
      ...
      //     return res.status(400).send({ status: false, message: "Please give pincode in a string" })
      //   }
      //   if (!Validation.isValidPinCode(pincode)) {
      //     return res.status(400).send({ status: false, message: "Length of the pin code should be 6" })
      //   }
